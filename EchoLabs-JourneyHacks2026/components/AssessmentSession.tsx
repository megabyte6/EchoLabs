...
const AssessmentSession: React.FC<AssessmentSessionProps> = ({ assessment, studentName, onFinish }) => {
        ...
        onmessage: async (message: any) => {
          ...
          if (message.serverContent?.turnComplete) {
            if (currentInputTranscription.current) {
              const entry: TranscriptEntry = { role: 'user', text: currentInputTranscription.current, timestamp: Date.now() };
              transcriptRef.current.push(entry);
              setTranscript([...transcriptRef.current]);
              currentInputTranscription.current = '';
            }
            if (currentOutputTranscription.current) {
              const entry: TranscriptEntry = { role: 'model', text: currentOutputTranscription.current, timestamp: Date.now() };
              transcriptRef.current.push(entry);
              setTranscript([...transcriptRef.current]);
  ...
  const finishAssessment = async () => {
    ...
    // Analyze transcript
    try {
      const analysis = await getAnalysis(transcriptRef.current, assessment.notes);
      ...
      const saved = localStorage.getItem('voxassess_results');
      const results = saved ? JSON.parse(saved) : [];
      localStorage.setItem('voxassess_results', JSON.stringify([result, ...results]));
    ...
    <div className="max-w-4xl mx-auto p-6 flex flex-col h-[calc(100vh-80px)]">
      {!isActive && !isAnalyzing && transcript.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center bg-white p-12 rounded-3xl border border-slate-200 shadow-2xl max-w-lg">
            ...
            <p className="text-slate-500 mb-8">
              Hi {studentName}, you are about to start <span className="font-bold text-slate-800">{assessment.title}</span>. 
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-800">Finalizing Results</h2>
          <p className="text-slate-500 mt-2 max-w-sm">
            AI is analyzing your conversation, counting filler words, and determining your predicted grade. 
            One moment please...
          </p>
        </div>
      ) : (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-500 font-bold uppercase flex items-center gap-1">
                  <span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping"></span> Live Session
                </p>
              </div>
            </div>
            <button 
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-lg flex items-center justify-between">
             <div className="flex items-center gap-4">
               <div className="flex gap-1">
                 {[1, 2, 3].map(i => (
                   <div key={i} className={`w-2 h-8 rounded-full bg-indigo-600 animate-pulse`} style={{animationDelay: `${i*0.1}s`, height: `${Math.random()*20 + 10}px`}}></div>
                 ))}
               </div>
               <p className="text-slate-500 font-medium italic">Gemini is listening...</p>
             </div>
